...
cd /tmp/
wget https://package-repository-skilrock.s3.ap-south-1.amazonaws.com/apache-tomcat-9.0.1_dms.tar.gz
mkdir /data
mkdir /data/DMS
cp  /tmp/apache-tomcat-9.0.1_dms.tar.gz /data/DMS/
sleep 1
cd /data/DMS/
/bin/tar -xzf /data/DMS/apache-tomcat-9.0.1_dms.tar.gz
chown -R root:root  /data/DMS/apache-tomcat-9.0.1
echo " " > /etc/systemd/system/dms.service
/bin/cat << EOM > /etc/systemd/system/dms.service
[Unit]
...
Type=forking
User=root
ExecStart=/data/DMS/apache-tomcat-9.0.1/bin/startup.sh run
ExecStop=/data/DMS/apache-tomcat-9.0.1/bin/shutdown.sh
...
chmod  755   /etc/systemd/system/dms.service
